  ...
  return (
    <header className='flex justify-between'>
      ...
      <div className="flex items-center justify-end gap-10">
        <button onClick={()=>props.setIsPopupOpen(prev=>!prev)} className='relative btn-cart cursor-pointer fill-[#69707D] hover:fill-very-dark-blue'>
          ...
          {props.productAttri.selected && <input type='text' disabled value={props.productAttri.quantity}  
            className='absolute top-0 -translate-y-1.5 w-4 text-white bg-orange rounded-full text-[10px] text-center'/>}
        </button>
